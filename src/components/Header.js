      ...
      <div className="container mx-auto flex items-center justify-between">
        ...
        <div className="lg:hidden">
          <button onClick={() => setMenuOpen(!menuOpen)}>
            {menuOpen ? (
              <FaTimes
                className={`${header ? "text-primary" : "text-white"} w-6 h-6`}
              />
            ) : (
              <FaBars
                className={`${header ? "text-primary" : "text-white"} w-6 h-6`}
        ...
        >
          ...
          <a
            href={`${baseUrl}/guest/login`}
            className="bg-accent text-white py-2 px-6 mt-4 lg:mt-0 lg:ml-4 lg:px-10 lg:py-2 lg:block"
          >
            Login
          </a>
        </nav>
